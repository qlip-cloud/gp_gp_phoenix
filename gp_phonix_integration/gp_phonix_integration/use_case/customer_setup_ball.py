...
@frappe.whitelist()
def sync_customer_ball(master_name):
...
def customer_save(customer_list, root_customer_group, default_country):
    ...
    if customer_new:
        ...
        add_contact_email = len(all_filter.get("list_contact_email_script"))  # len(all_filter.get("list_customer_script"))
...
def get_customer_list(company_name):
    ...
    customer_respose = execute_send(company_name = company_name, endpoint_code = SYNCUSTOMER, json_data = json_data)

    return customer_respose.get("Customers")
